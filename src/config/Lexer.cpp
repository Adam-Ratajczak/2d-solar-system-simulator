...
std::vector<Token> Lexer::lex() {
    ...
    while (true) {
        ...
        if (next == '\n') {
            consume_one();
            tokens.push_back(Token(Token::Type::NewLine, m_location, 1, "\n"));
            break;
        }
        ...
        if (next == '#') {
            ...
            std::string result = consume_while([&](char c) { return std::isalnum(c) || c == '_'; });
            tokens.push_back(Token(Token::Type::Identifier, start_location, m_location.index - start_location.index, result));
        }
        else if (next == '\"') {
            consume_one(); // "
            std::string result = consume_while([&](char c) { return c != '"'; });
            consume_one(); // "
