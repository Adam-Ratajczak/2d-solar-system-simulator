...
Textbox::Textbox(std::string text, sf::IntRect rect) : m_content(text), m_rect(rect){}
...
void Textbox::get_events(sf::Event& event){
    ...
    if(event.type == sf::Event::KeyReleased){
        con2 = 1;
    }else if(event.type == sf::Event::TextEntered && (timer.getElapsedTime().asMilliseconds() > 200 || con2)){
        timer.restart();
        con2 = 0;
